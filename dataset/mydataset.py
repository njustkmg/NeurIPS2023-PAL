...
def make_dataset(dir):
    dirs = os.listdir(dir)
    dirs = dict(enumerate(dirs))
    class_to_idx = {v: k for k, v in dirs.items()}
    # print(dirs)
    images = []
...
def make_dataset_idx(dir, class_to_idx):
    images = []
    dir = os.path.expanduser(dir)
    for target in os.listdir(dir):
        d = os.path.join(dir, target)
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, image_list, transform=None, target_transform=None, return_paths=False,
                 loader=default_loader,train=False, return_id=True):
...
class ImageFolder_fix(data.Dataset):
    ...
    def __init__(self, image_list, indexs=None, transform=None, target_transform=None, return_paths=False,
                 loader=default_loader,train=False, return_id=True):
        
        # image_list = make_dataset(image_list)
        imgs, labels = make_dataset_nolist(image_list)
        self.data = imgs
        self.targets= labels
...
class ImageFolder_fix_ex(data.Dataset):
    """A generic data loader where the images are arranged in this way: ::
        root/dog/xxx.png
        root/dog/xxy.png
        root/dog/xxz.png
